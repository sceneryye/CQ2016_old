...
class Magazine::TopicsController < Magazine::BaseController
  ...
  def index
    if params[:category_id].present?
        ...
        @topics = Topic.joins(:category).where("categories.name <> ?", "摩登报道")
    end
  ...
  def list
  	@topics = Topic.where(:cover_size=>"big", :published => true).paginate(:page => params[:page], :per_page => 9).order("created_at DESC")
