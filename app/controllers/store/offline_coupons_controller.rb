...
class Store::OfflineCouponsController < ApplicationController
	layout 'standard'
	def show
		@coupon  =  OfflineCoupon.includes(:brand).find(params[:id])
		return render :text=>"对不起,没有找到该优惠券!",:layout=>"standard" unless @coupon.published
		@brand = @coupon.brand
		@newin_vgoods = VirtualGood.where(:marketable=>true).order("uptime desc").limit(4)
		@maybe_coupons = OfflineCoupon.where("id <> ?", @coupon.id).limit(4)
	end
