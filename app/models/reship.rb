...
class Reship < ActiveRecord::Base
	...
	attr_accessor :reship_items, :province, :city, :district
	def reship_items=(items)
		items.each do |item|
			obj = ReshipItem.new(item)
	...
	def self.generate_reship_id
	    ...
	    seq = rand(10000)
          loop do
              ...
              return _reship_id  unless Reship.find_by_reship_id(_reship_id)
              seq = 1 if seq == 9999
              seq += 1
          end
	end
	...
	def update_order
		...
		reship_ids = Reship.where(:order_id=>self.order_id).pluck(:reship_id)
		reship_total = ReshipItem.where(:reship_id=>reship_ids).sum("number")
