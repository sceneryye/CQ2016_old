...
class Region < ActiveRecord::Base
	...
	belongs_to :province,->{where(region_grade:1)},:class_name=>"Region",:foreign_key=>"p_region_id"

	belongs_to :city,->{where(region_grade:2)},:class_name=>"Region",:foreign_key=>"p_region_id"
	...
	def self.hash_region
		Hash.new do |hash,key|
			hash[key] = Region.find_by_region_id(key).subregions.collect do |city|
