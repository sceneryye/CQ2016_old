...
class User < Base
  ...
  has_one :member_card, :foreign_key=>"member_id"
  ...
  has_many :favorites, ->{where(type:"fav") }, :foreign_key=>"member_id"
  ...
  def member_attr_value(col_name)
  	member_attr_values.where(:mr_id=>MemberAttr[col_name].mr_id).first
  end
  ...
  [:job,:income,:height,:weight,:shoesize,:price,:places,:colors,:edu,:interests,:voc].each do |ext_attr|
  	class_eval <<-ATTR_EVAL,__FILE__,__LINE__+1
  		def #{ext_attr}=(val)
                  return member_attr_values.where(:mr_id=>MemberAttr['#{ext_attr}'].mr_id).delete_all unless val.present?
  			val = val.join(',') if val.is_a?(Array)
  			return if v = member_attr_value('#{ext_attr}') and v.value == val
  			member_attr_values.where(:mr_id=>v.mr_id).delete_all if v
  			member_attr_values.build(:mr_id=>MemberAttr['#{ext_attr}'].mr_id,:value=>val)
  		end
  ...
  def gift
    mrid = MemberAttr['giftfornewuser'].mr_id
  ...
  def province_text
     p = Region.find_by_region_id(province)
     p.local_name if p
  end

  def city_text
     c = Region.find_by_region_id(city)
     c.local_name if c
  end

  def district_text
     d = Region.find_by_region_id(district)
     d.local_name if d
  end
  ...
  def parsed_region_ids
     ...
     region = Region.find_by_region_id(region_id.to_i)
  ...
  def check_mobile_duplicated
    if self.mobile.present? and u = User.find_by_mobile(self.mobile) and  u != self
      errors.add(:mobile, "手机号码已经被使用！")
    end
  end

  def check_email_duplicated
    if self.email.present? and u = User.find_by_email(self.email) and  u != self
      errors.add(:email, "邮箱已经被使用！")
    end
  end

 def receive_register_gift(val)
  	member_attr = MemberAttr.where(:tbl_name=>"sdb_b2c_members",
	 	      								:pk_name=>"member_id",
	 	      								:col_name=>"giftfornewuser").first
	 	      			

	member_attr_values = MemberAttrValue.where(:mr_id=>member_attr.mr_id,
													:pk=>self.member_id)
	MemberAttrValue.create(:mr_id=>member_attr.mr_id,
							    :pk=>self.member_id,
 ...
 def has_received_gift?
 	...
 	return true if MemberAttrValue.where(:mr_id=>member_attr.mr_id,:pk=>self.member_id).first
 	return false
 end

 def receive_coupon
 	#generate coupon
	coupon = Coupon.where(:cpns_prefix=>"B0001",:cpns_status=>"1").first
	coupon_code = coupon.make_coupon_code if coupon

	return if MemberCoupon.where(:member_id=>self.member_id,:memc_code=>coupon_code).size > 0
 ...
 def receive_new_coupon
    ...
    return if UserCoupon.where(:member_id=>self.member_id,:coupon_code=>coupon_code).present?

    UserCoupon.new do |uc|
 ...
 def update_validate
    ...
    u = User.find_by_member_id(self.member_id)
 ...
 def send_reset_password_sms
    ...
    self.reset_password_sent_at = Time.now
    save(:validate=>false)
    template = Config.get(:reset_password_sms_template)
