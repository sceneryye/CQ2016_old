...
class VirtualGood < ActiveRecord::Base
	...
	def self.import(csv_file,options={:encoding=>"GB18030:UTF-8"})
		...
		rows.each_with_index do |row,index|
			...
			row_error << "商品编号已经存在" if bn.present? && VirtualGood.find_by_bn(bn)
			...
			if cats_chain.present?
				...
				loop do
					...
					cats  = Category.where(:cat_name=>cat_name)
					if cats.size == 0
						row_error << "商品分类不存在"
						break
					else cats.size > 1
       ...
       # import data
		rows.each do |row|
			VirtualGood.new do |vgood|
				...
				_cats = Category.get_cat_by_names_chain(row[cat_index])
				vgood.cat_id = _cats.first.cat_id if _cats && _cats.first

				vgood.brand_id = Brand.find_by_brand_name(row[brand_index]).brand_id
				vgood.meta_seo = MetaSeo.new(:title=>row[title_index],
