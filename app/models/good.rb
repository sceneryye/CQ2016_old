...
class Good < ActiveRecord::Base
  ...
  has_many :image_attachs, ->{where(target_type:"goods")},  :foreign_key=>"target_id"

  has_many :images,	:through=>:image_attachs

  has_many :products, :foreign_key=>"goods_id",:class_name=>"Product",:dependent=>:destroy
  ...
  has_many :specs,->{uniq}, :through=>:good_specs

  has_many :good_spec_items,:foreign_key=>"goods_id"

  has_one :seo,->{where(mr_id:2) }, :foreign_key=>:pk

  has_one :good_collocation, :foreign_key=>"goods_id"

  has_many :comments,->{where(commentable_type:"goods" )}, :foreign_key=>"commentable_id"
  ...
  def collocation_goods
     return  self.good_collocation.collocations.collect do |goods_id|
        Good.find_by_goods_id(goods_id)
  ...
  def is_suit?
      cat = Category.where(:parent_id=>0,:cat_name=>SUIT_NAME).first
  ...
  def reco_collocation_goods
      cat = Category.find_by_cat_id(self.cat_id)
  ...
  def self.suits
      cat = Category.where(:parent_id=>0,:cat_name=>SUIT_NAME).first
      if cat
          # Good.where(:marketable=>'true',:cat_id=>cat.cat_id)
          Good.where("marketable = ? and cat_id = ? or p_50 = ?",'true',cat.cat_id,'true')
      else
          Good.where(:marketable=>'true')
      end
  end
  ...
  def color_specs(return_type='record')
     ...
     self.spec_desc[spec_color_id].collect do |private_spec_value_id,spec|
        if return_type == 'record'
          SpecValue.find(spec["spec_value_id"])
        else
  ...
  def list_default_pic
    ...
    #   list_pictures[0]
    # else
    #   pic = Image.find_by_image_id(self.image_default_id)
  ...
  def list_hover_pic
    if self.goods_id > NEW_GOOD_START_ID
      ...
      pic = Image.find_by_image_id(self.images_url)
      pic = self.images.order("attach_id desc").first unless pic
      pic = self.images.order("attach_id desc").last if  pic == Image.find_by_image_id(self.image_default_id)
  ...
  def favorited_by?(user)
    ...
    Favorite.where(:member_id=>member_id,
  ...
  def gift_image_url
    img = Image.find_by_image_id self.image_default_id
    "#{img.s_url}"
  end
