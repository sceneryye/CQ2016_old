...
class MemberAddr < ActiveRecord::Base
	...
	validates :mobile,:format=>{ :with=>/^\d{11}$/,:multiline => true,:message=>"手机号码必须是11位数字" },
	...
	def make_area
		_province = Region.find_by_region_id(province)
		_city = Region.find_by_region_id(city)
		_district = Region.find_by_region_id(district)
	...
	def update_member_def_addr
		MemberAddr.where("member_id = ? and addr_id <> ? ",self.member_id,self.addr_id).update_all(:def_addr=>0) if self.def_addr
	end
	...
	def parsed_region_ids
	     ...
	     region = Region.find_by_region_id(region_id)
